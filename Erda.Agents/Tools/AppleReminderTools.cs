...
/// tasks in his Reminders lists. A list is named by its real title, exactly as it reads in
/// Reminders.app; the bridge can reach all of them.
/// <para>
...
public sealed class AppleReminderTools(IAppleBridgeClient client)
{
    ...
    [Description(
        ...
        "milk to my groceries list'. Name the list by its real title as it appears in Reminders.app " +
        "(e.g. 'Groceries', 'Einkaufsliste') — capitalisation does not have to match exactly. There is " +
        "no default list, so ask Phil which list if unsure, or call list_apple_reminders with no list to " +
        "see what is there. A name that matches no list fails; never guess or fall back to a different " +
        "list.")]
    private async Task<string> CreateReminder(
        [Description("The Reminders list to add the task to, named as it reads in Reminders.app, e.g. 'Groceries'.")] string list,
        ...
        [Description("Optional priority: 0 = none (default), 1 = highest … 9 = lowest.")] int? priority = null)
    {
        if (string.IsNullOrWhiteSpace(list))
            return "Tell me which Reminders list to add this to.";
        if (string.IsNullOrWhiteSpace(title))
            return "Cannot create a reminder with no title.";

        var result = await client.CreateReminderAsync(list.Trim(), title.Trim(), notes, dueAt, priority);
        if (!result.Success)
